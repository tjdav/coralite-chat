...
const ONNX_MODELS = ['duration_predictor.onnx', 'text_encoder.onnx', 'tts.json', 'tts.yml', 'unicode_indexer.json', 'vector_estimator.onnx', 'vocoder.onnx']
const VOICE_STYLES = ['M1.json', 'M2.json', 'M3.json', 'M4.json', 'M5.json', 'F1.json', 'F2.json', 'F3.json', 'F4.json', 'F5.json']
...
fs.mkdirSync(onnxDir, {
  recursive: true
})
fs.mkdirSync(voiceStylesDir, {
  recursive: true
})
console.log('Downloading ONNX models...')
for (const model of ONNX_MODELS) {
  const destination = path.join(onnxDir, model)
...
for (const style of VOICE_STYLES) {
  const destination = path.join(voiceStylesDir, style)
